...
한 번만 하는 설정
깃허브(github.com) 무료 계정 생성
새 저장소(Repository) 하나 생성 — Public으로 (비공개로 하면 브라우저가 파일을 못 불러옴)
`detail-widget.css`, `detail-widget.js` 두 파일을 저장소에 업로드
저장소 Settings → Pages 들어가서 Source: Deploy from a branch, 브랜치는 `main`, 폴더는 `/(root)` 선택 → Save
잠시 후 `https://내계정.github.io/저장소이름/` 형태의 주소가 생성됨. 그 뒤에 파일명을 붙이면 해당 파일 주소가 됨
`https://inhyuk-alt.github.io/happycampusedu/detail-widget.css`
`https://inhyuk-alt.github.io/happycampusedu/detail-widget.js`
...
cssLink.href = 'https://inhyuk-alt.github.io/happycampusedu/detail-widget.css?cachebust=' + Date.now();
document.head.appendChild(cssLink);

fetch('https://inhyuk-alt.github.io/happycampusedu/detail-widget.js?cachebust=' + Date.now(), { cache: 'no-store' })
...
```
두 파일의 깃허브 주소는 12개 페이지 전부 완전히 동일해야 함
`window.PROGRAM_ID`만 페이지마다 다르게 (관리 웹앱에서 그 프로그램을 저장했을 때 나온 ID)
⚠️ 주의 1: JS는 `<script src="외부주소"></script>` 형태로 안 씁니다. 아임웹 코드위젯이
저장 시점에 이 형태의 태그를 걸러내서(실제로 겪은 문제), 저장 후 페이지에 아예 남지 않는
경우가 있었습니다. 그래서 `fetch`로 파일 내용을 텍스트로 받아온 뒤, 그 코드를 담은
`<script>` 요소를 직접 만들어 페이지에 끼워 넣는 방식으로 우회합니다.
⚠️ 주의 2: CSS도 일반 `<link href="...">`로 그냥 쓰면 브라우저가 예전 버전을 캐시해두고
계속 재사용해서, 깃허브에서 디자인을 고쳐도 시크릿 창을 열기 전엔 반영이 안 되는 경우가
있었습니다(실제로 겪은 문제 — 여백 값을 고쳤는데 그대로였던 사례). 그래서 CSS도 `<link>`
태그를 정적으로 쓰지 않고, 위 코드처럼 JS로 타임스탬프(`?cachebust=...`)를 붙여 직접
만들어 넣습니다. 이러면 새로고침 한 번으로 항상 최신 CSS가 반영됩니다.
디자인을 고치고 싶을 때 (앞으로 계속 반복되는 작업)
깃허브 사이트에 로그인
`detail-widget.css` 클릭 → 연필(edit) 아이콘 클릭
값 수정 → 아래 초록색 Commit changes 버튼
대략 몇 십 초~1분 안에 12개 페이지 전부에 자동 반영 (위 캐시 무력화 처리 덕분에 시크릿 창 없이 일반 새로고침만으로 확인 가능)
...
```json
{
  "subtitleLabel": "라벨명 (선택, 예: 한 줄 소개 / 핵심 요약 등 자유 입력)",
  "subtitleText": "제목 바로 아래 표시될 짧은 소개 문구 (선택)",
  "regions": [
    {
      "name": "지역/장소명",
      "nameColor": "#RRGGBB (선택)",
...
[한 줄 소개] 관련 참고 (2026-07 추가): `subtitleLabel`/`subtitleText`는 관리 화면의
제목 입력 구역에서 함께 입력합니다. `subtitleText`가 비어 있으면 실제 페이지에서 이
영역 자체가 표시되지 않습니다. 입력창은 줄바꿈이 안 되는 한 줄짜리라, 내용이 길면
페이지에서는 폭에 맞춰 자동으로 다음 줄로 넘어갑니다(글자 크기는 고정 유지).
이 저장소로 안 되는 것 (관리 웹앱에서 해야 하는 일)
프로그램 추가/삭제, 제목·사진·카테고리·한 줄 소개 변경
지역/일정/달력 날짜 입력
모집인원/주요내용/문의/추가정보 내용 입력
이런 내용(콘텐츠) 변경은 전부 별도의 관리 웹앱(구글 시트 기반)에서 합니다.
이 저장소는 오직 화면 디자인과 렌더링 로직만 담당합니다.
참고: 반영 지연
이 저장소(깃허브 페이지) 수정 → 보통 1분 이내 반영, 캐시 무력화 처리 덕분에 일반
새로고침만으로 바로 확인 가능 (시크릿 창 열 필요 없음)
